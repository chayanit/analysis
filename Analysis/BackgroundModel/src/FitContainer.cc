...
FitContainer::FitContainer(const std::string& outputDir) :
		...
		obs_(105053.) //SR1-259399, SR2-105053, SR3-26760
{}
...
//			: FitContainer(container.data().get(), container.bbH().get(), container.background().get(), outputDir) {
			: FitContainer(container.data().get(), outputDir) {
}
...
std::unique_ptr<RooFitResult> FitContainer::backgroundOnlyFit(const std::string& name) {
  ...
  getNumbersPerBins_(*frame, "background_curve", "data_curve", hpull);
...
void FitContainer::getNumbersPerBins_(const RooPlot& frame, const char* curvename, const char* histname, RooHist* hpull)
{
  // Find curve object
  ...
  double x,y_data,y_fit,y_pull;
  double dyl,dyh;

  for(int i = 0; i < hist->GetN(); i++){
	hist->GetPoint(i,x,y_data);
	//curve->GetPoint(i,x,y_fit);
	hpull->GetPoint(i,x,y_pull);
	y_fit = curve->interpolate(x);

	dyl = hist->GetErrorYlow(i);
  	dyh = hist->GetErrorYhigh(i);

  std::cout << "bin " << i << " : M12 = " << x << "\t data = " << y_data << "\t fit = " << y_fit << "\t data-fit = " << y_data-y_fit << "\t error ylow = " << dyl << "\t error yhigh = " << dyh << "\t pull = " << y_pull << std::endl;
